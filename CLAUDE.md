...
- **Alphabet pronunciation ladder** (`app.js`, `RU_PHONICS` + `ruPronStages`/`ruPronounce*`): a 6-stage A1 program — Sounds → Syllables → Stress → Tricky pairs → A1 words → A1 phrases. **Every stage is mic-graded on a clarity scale** by `ruAssess()` from the Web Speech API result: 🟢 Clear (your target was the recognizer's top guess) / 🟡 Close (recognised, lower alternative or within edit-distance) / 🔴 Off (not recognised) — plus a `ruNearMiss()` hint naming the first differing sound. **No self-marking** (removed on user feedback). Clear/Close count as practised; passes persist in `localStorage` (`ru_pron_passed`, keyed `"<stage>:<show>"`); the Phase-0 chip shows overall "X/Y practised". Recognition is still transcription-based (intelligibility proxy, not true phoneme scoring). The alphabet is **excluded from the generic SM-2 drill**: `russian_drills` drops phase 0 when scope is "all", and `russian_stats.due_today` counts only `phase > 0`.
...
- **Lessons tab** (📚): `RU_GRAMMAR` (~9 A1 grammar lessons — teaching text + examples + a multiple-choice `ruRenderCheck`) and `RU_READING` (~6 graded passages — tappable words → `ruReadWord` glosses+speaks into a sticky gloss bar, `ruToggleTranslation`, comprehension MC). One tab with a Grammar/Reading toggle (`ruRenderLessons`/`ruLessonsView`); menu→detail like chapters. Progress in `localStorage` (`ru_lessons_done`, keyed `"g:<id>"`/`"r:<id>"`). Zero-API, no backend.
