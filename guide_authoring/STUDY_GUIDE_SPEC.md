...
- **Unified style system (2026-09, baked into `_template.html` — new guides inherit it, nothing to wire):**
  ...
  - **Active-recall checklists** — the `<b>` key terms in each `.recall` item auto-become tap-to-reveal cloze blanks with a progress bar (`sg-recall-js` — no per-guide wiring). **Write each item as `<b>cue label</b> — <b>answer</b>`:** a *leading* `<b>` label immediately followed by a separator (`—`, `=`, `:`, `→`) is kept **visible as the question cue** (class `.rcue`), and only the *remaining* `<b>`s become blanks — so every line clearly asks something instead of hiding the label. Never leave an item with only the label bold (it would blank the cue); bold the answer too. Items with a single `<b>` mid-sentence still just blank that term.
